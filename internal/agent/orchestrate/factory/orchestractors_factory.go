...
func AbstractOrchestratorsFactory(
	orchestratorType string,
	collector collect.Collector,
	producer produce.Producer,
	logger *zap.SugaredLogger,
) (orchestrate.OrchestratorAbstractFactory, error) {
	switch orchestratorType {
	case OrchestratorTypeBasic:
		return basic.NewOrchestratorFactory(collector, producer, logger), nil
	default:
		return nil, fmt.Errorf(
			"unsupported orchestrator type: '%s', please provide a valid orchestrator type",
			orchestratorType,
		)
	}
}
