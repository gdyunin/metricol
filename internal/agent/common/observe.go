...
// Unsubscribe removes the given observer from the subject's list of observers.
// It calls the subject's RemoveObserver method and returns an error if the unsubscription fails.
func Unsubscribe(observer Observer, subject ObserveSubject) error {
	if err := subject.RemoveObserver(observer); err != nil {
		return fmt.Errorf("failed to unsubscribe observer from subject: %w", err)
	}
	return nil
}
